...
class Spliter:
    ...
    def get_split(self):
        ...
        logger.info(
            f"[spliter summary] (splits {self.splits}) train {len(train)} val {len(val)} test {len(test)}"
        )
    ...
    def _setup_split_index(self):
        ...
        path = os.path.join(
            self._index_save_path, f"spliting_{self.dataset.key_name}_{self.splits}.csv"
        )
...
class IndexSpliter:
    ...
    def get_split(self):
        ...
        logger.debug(
            f"[spliter summary] (splits {self.splits}) train {len(train_index)} val {len(val_index)} test {len(test_index)}"
        )
    ...
    def _setup_split_index(self):
        ...
        if not os.path.isfile(path) or not self.deterministic:
            ...
            if len(df_split_index) != len(self.labels):
                logger.debug(
                    f"[spliter] df_split_index and labels length not match {len(df_split_index)} != {len(self.labels)}"
                )
                df_split_index = self.split_and_save_index(path)
            else:
                logger.debug(
                    f"[spliter] Read index from file, length of index {len(df_split_index)}"
                )
