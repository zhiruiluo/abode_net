...
class FCN_Base(nn.Module):
    ...
    def fcnzl_out(self, var_len, time_len):
        for i in range(self.num_layers):
            var_len = self.conv_out_shape(
                var_len, self.padding_list[i][0], self.stride_list[i][0], self.ker_list[i][0], 1
            )
            time_len = self.conv_out_shape(
                time_len, self.padding_list[i][1], self.stride_list[i][1], self.ker_list[i][1], 1
            )
...
class FCNZL2D(FCN_Base):
    ...
    def __init__(self, in_channels, bn=True):
        super().__init__(
            in_channels, self.chan_list, self.ker_list, self.padding_list, self.stride_list, bn
        )
...
class ABODE_Net_model(nn.Module):
    def __init__(self, nclass) -> None:
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(1), nn.Dropout(0.4), spectral_norm(nn.Linear(in_classifier, nclass))
        )
