        ...
        def BufferWrapper(fn, *args, **kwargs):
            ...
            def run_and_write():
                ...
                if not disable:
                    ...
                    logger.debug(
                        f"Writer object to {fpath}, @{protocol}, spent time {time()-t1:2.4f} sec"
                    )
                return out
            ...
            if not os.path.isfile(fpath) or disable:
                ...
                try:
                    ...
                    if prev_func_code != func_code:
                        logger.debug(
                            f"function_code change detected! {len(prev_func_code)} {len(func_code)} {prev_func_code != func_code}"
                        )
                        rerun_flag = True
                    else:
                        logger.debug(
                            f"Read object from {fpath} in FUNC {func.__name__} {inspect.getfile(func)} #{inspect.currentframe().f_back.f_lineno}, @{protocol}, spent time {time()-t1:2.4f} sec"
                        )
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        def run_and_write():
            ...
            if not self.disable:
                ...
                writer((func_code, out), fpath)
                logger.debug(
                    f"Writer object to {fpath}, @{self.protocol}, spent time {time()-t1:2.4f} sec"
                )
            return out
        ...
        if fpath.is_file() or self.disable:
            ...
            try:
                ...
                if prev_func_code != func_code:
                    ...
                    logger.debug(
                        f"Read object from {fpath} in FUNC {self.func.__name__} {inspect.getfile(self.func)} #{inspect.currentframe().f_back.f_lineno}, @{self.protocol}, spent time {time()-t1:2.4f} sec"
                    )
            except Exception as e:
                logger.debug(f"[buffer_value] {traceback.format_exc()}. Rerun!")
                rerun_flag = True
...
def timeit(func):
    def timer_wrapper(*args, **kwargs):
        ...
        out = func(*args, **kwargs)
        logger.info(
            f"Function {func.__name__} args: [{args}, {kwargs}] spent time {time()-t1:2.4f} sec"
        )
        return out
