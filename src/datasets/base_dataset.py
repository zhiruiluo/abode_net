...
def find_class(labels: Union[np.ndarray, List]):
    ...
    class_to_index = {classname: i for i, classname in enumerate(classes)}
...
class BaseDataset(Dataset):
    ...
    def _find_class_(
        self, labels: Union[np.ndarray, List], one_hot: bool
...
class DictDataset(Dataset):
    ...
    def __getitem__(self, index):
        return {
            "input": self.to_tensor(self.input[index], dtype=torch.float32),
            "target": self.to_tensor(self.target[index], dtype=torch.long),
        }
...
class BaseSplitDataset(BaseDataset):
    ...
    def get_split(self):
        ...
        logger.info(
            f"x_train {x_train.shape} label_train {label_train.shape} x_test {x_test.shape} label_test {label_test.shape}"
        )
        return (
            TensorDataset(x_train, label_train),
            TensorDataset(x_test, label_test),
            TensorDataset(x_test, label_test),
        )
