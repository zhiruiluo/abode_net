...
def load_data(root, study_case):
    scenarios = {'case1': ['Home_A','spring'], 'case2': ['Home_A','summer'], 'case3': ['Home_B', 'summer']}
...
def preprocess_household_data(household_data):
    ...
    df_by_date = [(pd.Timestamp(index), x.drop(['date'],axis=1)) for index, x in df.groupby('date')]
...
def prepare_data(household_data,fea_type,agg_sec,slide_param,addtional_features):
    ...
    agg_features = day_wrapper(df_by_date ,'energy', features, agg_time=pd.Timedelta(agg_sec,'s'), slide_param=slide_param)
    agg_occ = day_wrapper(df_by_date, 'occ', ['occ'], agg_time=pd.Timedelta(agg_sec,'s'), slide_param=slide_param)
...
class NIOMDataset(pl.LightningDataModule):
    def __init__(self, study_case,norm_type,splits,fea_type,data_aug,agg_sec,
        ...
        self.threshold = 0.5
        if t_en: self.addtional_features.append('time')
    ...
    def on_prepare_split(self):
        self.spliter = NIOM_Spliter(self.dataname, self.labels, self.splits, self.nrepeat, deterministic=self.determ)
...
def test():
    ...
    keys_dict = dict(study_case='case1',norm_type='minmax',splits='3:1:1',fea_type='raw',
        data_aug='SMOTE',agg_sec=60,batch_size=32,random_state=42,imb_sam=1,slide_w=60,
        slide_s=60,t_en=1,w_en=1,hol_en=0,deterministic=1
        )
