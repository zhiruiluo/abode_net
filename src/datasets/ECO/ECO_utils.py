...
def extract_zip(input_zip):
    ...
    return {name: input_zip.read(name) for name in input_zip.namelist() if ('.csv' in name) & ('__MACOSX' not in name)}
...
def load_data(root, household, period):
    zip_files = [f'{household}_sm_csv.zip',f'{household}_occupancy_csv.zip',f'{household}_plugs_csv.zip']
    house_hold_df = {}
    for f in zip_files:
...
def retrive_study_case(case):
    cases = {'case1': {'household': '01','period': 'summer','threshold': 0.5,},
            'case2': {'household': '01','period': 'winter','threshold': 0.5,},
            'case3': {'household': '02','period': 'winter','threshold': 0.5,},
            'case4': {'household': '03','period': 'summer','threshold': 0.5,}}
...
def aggregate_sliding_window_by_date(df, aggregation_seconds, slide_winsize, slide_stride):
    time_range = pd.date_range('00:00:00', periods=24*60*60/aggregation_seconds, freq=f'{aggregation_seconds}s').strftime('%H:%M:%S')
    if df is None:
...
def aggregate_appliance(appl_comm, aggregation_seconds, slide_winsize, slide_stride, cumsum):
    ...
    for date, df_appl_by_date in tqdm(appl_comm):
        ...
        all_dates.append(aggregate_sliding_window_by_date(features, aggregation_seconds, slide_winsize, slide_stride))
...
def aggregate_occupancy(df_occ_comm, aggregation_seconds, slide_winsize, slide_stride, th):
    ...
    for date in df_occ_comm:
        ...
        occ = aggregate_sliding_window_by_date(occ_by_date, aggregation_seconds, slide_winsize, slide_stride)
...
def aggregate_sm_cwt(sm_comm, aggregation_seconds, sliding_window, f_time, f_week):
    ...
    for date, sm_df_by_date in tqdm(sm_comm):
        ...
        four_features = rearrange(temp_df.groupby(['group']).mean().to_numpy(), '(n w) v -> n v w', w=sliding_window)
        cwt_feature = []
        for n in four_features:
            samp_fea = []
            for fea in n:
                samp_fea.append(signal.cwt(fea, signal.ricker, widths=np.arange(1,aggregation_seconds+1)))
        ...
        if f_week:
            ...
            df_weekday = pd.DataFrame([weekday]*24*60*60,columns=['weekday'],index=temp_df.index)
            df_weekday['group'] = tile
            week_feature = rearrange(df_weekday.groupby(['group']).max().to_numpy(), '(n w) 1 -> n w', w=sliding_window)
...
def aggregate_sm(sm_comm, feature_type, aggregation_seconds, slide_winsize, slide_stride, f_time, f_week, f_hol, f_fix, f_prob, f_cumsum):
    all_features = []
    for date, sm_df_by_date in tqdm(sm_comm):
        # temp_df shape (t v)
        temp_df = sm_df_by_date.loc[:, 0:0].copy()
        ...
        if f_time:
            t = np.arange(24*60*60); t = t/np.max(t)
        ...
        if f_week:
            ...
            df_weekday = pd.DataFrame(np.array([weekday]*24*60*60),columns=['weekday'], index=temp_df.index)
            features.append(group_feature(df_weekday, tile, 'max'))
        if f_hol:
            swiss_holidays = holidays.Switzerland(); hol = int(date in swiss_holidays)
            df_hol = pd.DataFrame(np.array([hol]*24*60*60), columns=['holiday'], index=temp_df.index)
        ...
        if f_prob:
            pass
        if f_cumsum:
            df_cumsum = sm_df_by_date.loc[:, 0:0].copy().cumsum()
...
def preprocess(household_data, threshold, feature_type, aggregation_seconds, slide_winsize, 
        slide_stride, f_time, f_week, f_hol, f_fix, f_prob, f_plugs, plugs_cum, sm_cum
    ):
        ...
        for k in household_data:
            if 'plugs' in k:
                ...
                if len(appl_dates_inter_occ) == 0:
                    ...
                    continue
                appl_comm = [(date, dict_appl[date.strftime('%Y-%m-%d')]) if date.strftime('%Y-%m-%d') in dict_appl else (date, None) for date in intersect_occ_sm_days]
                # agg_appl (n t 1)
                agg_appl = aggregate_appliance(appl_comm, aggregation_seconds, slide_winsize, slide_stride, plugs_cum)
    ...
    if feature_type == 'cwt':
        # (n 60 v) -> (n 60 60)
        agg_sm = aggregate_sm_cwt(sm_comm, aggregation_seconds, slide_winsize, slide_stride, f_time, f_week)
    else:
        agg_sm = aggregate_sm(sm_comm, feature_type, aggregation_seconds, slide_winsize, slide_stride, f_time, f_week, f_hol, f_fix, f_prob, sm_cum)
        agg_sm = rearrange(agg_sm, 'n t v -> n 1 v t')

    agg_occ = aggregate_occupancy(df_occ_comm, aggregation_seconds, slide_winsize, slide_stride, th=threshold)
